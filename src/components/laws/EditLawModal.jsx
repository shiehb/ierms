...
export default function EditLawModal({ law, onClose, onLawUpdated }) {
  ...
  const handleReferenceCodeValidation = async (code) => {
    ...
    // Don't check if code is empty or same as original
    if (!code || code.trim().length === 0 || code === law?.reference_code) {
      setCodeExists(false);
      return;
    }
  ...
  const confirmEdit = async () => {
    ...
    setLoading(true);
    try {
      const updatedLaw = await lawApi.updateLaw(law.id, formData);
      ...
      if (onLawUpdated) onLawUpdated(updatedLaw);
      onClose();
    } catch (error) {
    ...
    return (
      <div className="space-y-3">
        <p className="text-gray-700">
          Do you wish to proceed with updating this law?
        </p>
          ...
          <div>
            ...
            <div className="relative">
              ...
              {!codeExists &&
                !validatingCode &&
                formData.reference_code.trim() &&
                formData.reference_code !== law?.reference_code && (
                  <div className="absolute right-3 top-3">
                    <svg
          ...
          <div>
            ...
            <div className="relative">
              ...
              {!titleExists &&
                ...
                formData.law_title.trim() &&
                formData.law_title.trim().length >= 3 &&
                formData.law_title !== law?.law_title && (
                  <div className="absolute right-3 top-3">
                    <svg
                      className="w-5 h-5 text-green-500"
                      fill="currentColor"
